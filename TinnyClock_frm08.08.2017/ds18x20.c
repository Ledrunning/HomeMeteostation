...
uint8_t DS18x20_ReadData(uint8_t *rom, uint8_t *buffer)
{
	...
	if (crc8(buff, 9)) return 0;	// If the checksum does not match, it returns an error;
#else 
...
void DS18x20_ConvertToThemperature(uint8_t* data, uint8_t* themp)
{
	...
	{
		themp[0] = 127-themp[0];
		themp[0] |= 0b10000000; // If the temperature is below zero;
	} 
}
...
float DS18x20_ConvertToThemperatureFl(uint8_t* data)
{
	...
	if (data[1]>0xFB){
		digit = 127-digit;
		digit |= 0b10000000; // If the temperature is below zero;
	} */
	return Temperature;
}
