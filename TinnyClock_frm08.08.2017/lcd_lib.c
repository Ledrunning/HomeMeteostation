...
//*****************************************************************************
void lcd_init(void) // Display initialize;
{
	_delay_ms(15);
	LDP=0x00;
	LCP=0x00;
	LDDR|=1<<LCD_B7|1<<LCD_B6|1<<LCD_B5|1<<LCD_B4;
	LCDR|=1<<LCD_E|1<<LCD_RW|1<<LCD_RS;
   //---------one------;
	LDP=0<<LCD_B7|0<<LCD_B6|1<<LCD_B5|1<<LCD_B4; //4 bit mode;
	...
	_delay_ms(1);
	//-------three-------------;
	LDP=0<<LCD_B7|0<<LCD_B6|1<<LCD_B5|0<<LCD_B4; //4 bit mode;
	LCP|=1<<LCD_E|0<<LCD_RW|0<<LCD_RS;		
	_delay_ms(1);
	LCP&=~(1<<LCD_E);
	_delay_ms(1);
...
//*****************************************************************************
void lcd_string(uint8_t* data, uint8_t nBytes)	// String to display;
{
register uint8_t i;
...
//*****************************************************************************
void lcd_gotoxy(uint8_t x, uint8_t y)	// Move the cursor to the position XY;
{
...
//*****************************************************************************
// The output from the memory string in the XY position
...
{                       // bar level, max bar level, lenght of the bar;
	uint8_t i;
